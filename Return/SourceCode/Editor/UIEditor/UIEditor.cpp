...
CUIEditor::CUIEditor()
	: m_pUI				( nullptr )
	, m_DispScene		( ESceneList::Title )
	, m_UIScene			( ESceneList::Title )
	, m_OldBackColor	( CDirectX11::GetBackColor() )
...
{
	// 背景の色を元に戻す.
	CDirectX11::SetBackColor( m_OldBackColor );

	// シーン情報を保存する.
	m_IsCreaterLog = false;
	if ( m_IsAutoSave ) {
		if ( FAILED( m_pUI->AllSpriteStateDataSave() ) ) return;
		CLog::PushLog( "自動で" + StringConversion::Enum( m_DispScene ) + "シーンのスプライト情報を全て保存しました。" );
	}
...
{
	// UIの作成.
	m_pUI = std::make_unique<CTitleWidget>();
	m_pUI->Init();
	m_pUI->SetIsCreaterLog( &m_IsCreaterLog );
	m_pUI->SetLogList( &m_UndoLogList );

	// 背景のグリッド線の取得.
	m_pGrid = CSpriteResource::GetSprite( "EdtorGrid" );
...
void CUIEditor::Update( const float& DeltaTime )
{
	...
	// 保存.
	if ( CKeyInput::IsANDKeyDown( VK_CONTROL, 'S' ) ) Save();
	...
	// 背景色の設定.
	CDirectX11::SetBackColor( m_BackColor );
}
...
void CUIEditor::ImGuiRender()
{
	...
	if ( ImGui::BeginTabBar( "TabBar", ImGuiTabBarFlags_FittingPolicyScroll | ImGuiTabBarFlags_TabListPopupButton ) ) {
		...
		if ( ImGui::BeginTabItem( "Scene" ) ) {
			...
			CImGuiManager::Combo( u8"表示シーン設定", &m_DispScene, { ESceneList::None, ESceneList::Max, ESceneList::UIEdit } );
			ImGui::SameLine();
			// 現在のシーンのスプライトの全て保存するボタン.
			if ( CImGuiManager::Button( "AllSave" ) ) Save();
			ImGui::SameLine();
			// リセットボタン.
			if ( CImGuiManager::Button( "Reset" ) ) {
				// 一時的にオートセーブを無効にする.
		...
		if ( ImGui::BeginTabItem( "NewUI" ) ) {
			// 読み込んでる全てのUIのコンボボックス.
			CImGuiManager::Combo( u8"UIリスト", &m_SelectUI, CSpriteResource::GetSpriteNames() );
			ImGui::SameLine();
			// スプライトの追加ボタン.
			if ( CImGuiManager::Button( "Add" ) ) {
				CSprite* pUI = CSpriteResource::GetSprite( m_SelectUI );
				pUI->AddDispSprite( m_IsAutoSave, StringConversion::Enum( m_DispScene ) );
				m_pUI->AddUISpreite( m_SelectUI, pUI );
			}
			ImGui::Separator();
			const SSpriteState& State = CSpriteResource::GetSprite( m_SelectUI )->GetSpriteState();
			// 画像情報.
			...
			ImGui::Image( CSpriteResource::GetSprite( m_SelectUI )->GetTexture(), ImVec2( State.Base.w * n, State.Base.h * n ) );
			ImGui::EndChild();
			ImGui::EndTabItem();
		}
		// オプションタブ.
		if ( ImGui::BeginTabItem( "Option" ) ) {
			...
			ImGui::SameLine();
			CImGuiManager::HelpMarker( u8"「-1」の場合ログファイルの上限は無しとして扱います。" );
			ImGui::Separator();

			// リセットされるタイミングで自動で保存するかの設定.
			ImGui::Text( u8"自動で保存されるようにするか" );
			CImGuiManager::CheckBox( "##IsAutoSave", &m_IsAutoSave );
			ImGui::Separator();

			// ログファイルを作成するかの設定.
			ImGui::Text( u8"ログファイルを作成するか" );
			CImGuiManager::CheckBox( "##IsCreaterLog", &m_IsCreaterLog );
			ImGui::Separator();

			// 当たり判定を表示するかの設定.
			ImGui::Text( u8"マウスで画像を移動させれるか" );
			CImGuiManager::CheckBox( "##IsDrag", &m_IsDrag );
			ImGui::Separator();

			// 当たり判定を表示するかの設定.
			ImGui::Text( u8"当たり判定を表示するか" );
			CImGuiManager::CheckBox( "##IsDispHitBox", &m_IsDispHitBox );
			ImGui::Separator();

			// 当たり判定を表示するかの設定.
			ImGui::Text( u8"閉じたときにファイルを削除するか" );
			CImGuiManager::CheckBox( "##IsFileDelete", &m_IsFileDelete );
			ImGui::Separator();

			// UIの更新を行うかの設定.
			ImGui::Text( u8"UIの更新を行うか" );
			CImGuiManager::CheckBox( "##IsWidgetUpdate", &m_IsWidgetUpdate );
			ImGui::SameLine();
			CImGuiManager::HelpMarker( u8"有効にした場合、バグる可能性があります。" );
			ImGui::Separator();
...
void CUIEditor::ChangeDispScene( const bool IsSaveStop )
{
	...
	case ESceneList::GameClear:
		m_pUI = std::make_unique<CGameClearWidget>();
		break;
	default: return;
	}
	m_pUI->Init();
...
{
	if ( FAILED( m_pUI->AllSpriteStateDataSave() ) ) return;
	CMessageWindow::PushMessage( u8"全て保存しました。(Ctrl + S)", Color4::Magenta );
}
...
void CUIEditor::Undo()
{
	...
	if ( CKeyInput::IsANDKeyDown( VK_CONTROL, 'Z' ) == false ) return;
	...
	CMessageWindow::PushMessage( u8"元に戻しました。(Ctrl + Z)", Color4::Magenta );
}
...
void CUIEditor::Redu()
{
	...
	if ( CKeyInput::IsANDKeyDown( VK_CONTROL, 'Y' ) == false ) return;
	...
	// オートセーブを無効にして読み込み直し.
	ChangeDispScene( true );
	CMessageWindow::PushMessage( u8"やり直しました。(Ctrl + Y)", Color4::Magenta );
}
