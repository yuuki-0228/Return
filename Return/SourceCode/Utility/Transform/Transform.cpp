...
// 度数で回転を取得.
//---------------------------.
D3DXPOSITION2 stTransform::Get2DPosition() const
{
	return D3DXPOSITION2( Position.x, Position.y );
}

//---------------------------.
// 度数で回転を取得.
//---------------------------.
D3DXROTATION3 stTransform::GetDegRotation()
{
...
// ラジアンで回転を取得する.
//---------------------------.
D3DXROTATION3 stTransform::GetRadRotation()
{
...
D3DXPOSITION3 stTransform::GetWorldPosition( const D3DXMATRIX* pRot ) const
{
	...
	D3DXMATRIX mWorld, mScale, mRot, mTran, mLastScale, mParRot, mParTran;
	...
	// 最終の拡縮行列作成.
	const D3DXSCALE3& LastScale = pParent->GetWorldScale();
	D3DXMatrixScaling( &mLastScale, LastScale.x, LastScale.y, LastScale.z );
	// 親の中心回転行列作成.
	...
	// 親を含めたワールド行列を作成.
	mWorld = mLastScale * mRot * mTran * mParRot * mParTran;
...
D3DXMATRIX stTransform::GetWorldMatrix( const D3DXMATRIX* pRot )
{
	...
	if ( pParent == nullptr ) return mScale * mRot * mTran;
		...
		D3DXMATRIX mLastScale, mParRot, mParTran;
		...
		// 親を含めたワールド行列を作成.
		return mRot * mTran * mLastScale * mParRot * mParTran;
	}
}
