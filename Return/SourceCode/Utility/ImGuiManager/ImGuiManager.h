...
private:
	...
	CImGuiManager( CImGuiManager && )					= delete;
	CImGuiManager& operator = ( CImGuiManager && )		= delete;
};
...
//-----------------------------.
template<class T>
T CImGuiManager::RadioButton( const char* Label, T* Out, std::vector<T> SkipEnum )
{
	...
	if ( *Out != static_cast<T>( v ) ) CSoundManager::PlaySE( "ImguiSE" );
...
//-----------------------------.
template<class T>
T CImGuiManager::Combo( const char* Label, T* Out, std::vector<T> SkipEnum )
{
	int v = static_cast<int>( *Out );
