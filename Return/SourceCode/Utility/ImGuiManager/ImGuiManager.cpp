...
HRESULT CImGuiManager::Init( HWND hWnd )
{
	...
	if ( ImGui_ImplDX11_Init( CDirectX11::GetDevice(), CDirectX11::GetContext() ) == false ) return E_FAIL;

	CLog::PushLog( "Imguiの初期化 : 成功" );
...
void CImGuiManager::Render()
{
	...
	if ( pI->m_IsRender == true ) {
		...
		CSystemWindowManager::Render();
		CMessageWindow::Render();
		CDebugFlagWindow::Render();
		CDebugWindow::Render();
...
//	描画していない場合は処理は行わない.
//----------------------------.
void CImGuiManager::PushRenderProc( const std::function<void()>& Proc )
{
	CImGuiManager* pI = GetInstance();
...
// スライダーでint型の表示.
//----------------------------.
void CImGuiManager::SliderInt( const char* Name, int* i, const int Min, const int Max, const int Init )
{
	...
	if ( ImGui::TreeNode( Name ) ) {
		...
		// 初期化ボタン.
		if ( CImGuiManager::Button( "Reset" ) ) *i = Init;
		ImGui::SameLine();
...
// 入力ボックスでint型の表示.
//----------------------------.
void CImGuiManager::InputInt( const char* Name, int* i, const int Init )
{
	ImGui::SetNextTreeNodeOpen( true, ImGuiCond_Once );
	if ( ImGui::TreeNode( Name ) ) {
		// ボックスの表示.
...
//----------------------------.
void CImGuiManager::SliderInputInt( const char* Name, int* i, const int Min, const int Max, const int Init )
{
	ImGui::SetNextTreeNodeOpen( true, ImGuiCond_Once );
	if ( ImGui::TreeNode( Name ) ) {
		// スライダーの表示.
...
// スライダーでfloat型の表示.
//----------------------------.
void CImGuiManager::SliderFloat( const char* Name, float* f, const float Min, const float Max, const float Init )
{
	ImGui::SetNextTreeNodeOpen( true, ImGuiCond_Once );
	if ( ImGui::TreeNode( Name ) ) {
		// スライダーの表示.
		...
		// 初期化ボタン.
		if ( CImGuiManager::Button( "Reset" ) ) *f = Init;
		ImGui::SameLine();
		HelpMarker( u8"初期状態に戻します" );
		ImGui::TreePop();
	}
...
// 入力ボックスでfloat型の表示.
//----------------------------.
void CImGuiManager::InputFloat( const char* Name, float* f, const float Init )
{
	ImGui::SetNextTreeNodeOpen( true, ImGuiCond_Once );
	if ( ImGui::TreeNode( Name ) ) {
		// ボックスの表示.
...
//----------------------------.
void CImGuiManager::SliderInputFloat( const char* Name, float* f, const float Min, const float Max, const float Init )
{
	ImGui::SetNextTreeNodeOpen( true, ImGuiCond_Once );
	if ( ImGui::TreeNode( Name ) ) {
		// スライダーの表示.
...
//----------------------------.
void CImGuiManager::SliderVec2( const char* Name, D3DXVECTOR2* Vec2, const float Min, const float Max, const D3DXVECTOR2& Init )
{
	ImGui::SetNextTreeNodeOpen( true, ImGuiCond_Once );
	if ( ImGui::TreeNode( Name ) ) {
		// スライダーの表示.
		...
		// 初期化ボタン.
		if ( CImGuiManager::Button( "Reset" ) ) *Vec2 = Init;
		ImGui::SameLine();
		HelpMarker( u8"初期状態に戻します" );
		ImGui::TreePop();
	}
...
//----------------------------.
void CImGuiManager::InputVec2( const char* Name, D3DXVECTOR2* Vec2, const D3DXVECTOR2& Init )
{
	ImGui::SetNextTreeNodeOpen( true, ImGuiCond_Once );
	if ( ImGui::TreeNode( Name ) ) {
		// ボックスの表示.
...
//----------------------------.
void CImGuiManager::SliderInputVec2( const char* Name, D3DXVECTOR2* Vec2, const float Min, const float Max, const D3DXVECTOR2& Init )
{
	ImGui::SetNextTreeNodeOpen( true, ImGuiCond_Once );
	if ( ImGui::TreeNode( Name ) ) {
		// スライダーの表示.
...
//----------------------------.
void CImGuiManager::SliderVec3( const char* Name, D3DXVECTOR3* Vec3, const float Min, const float Max, const D3DXVECTOR3& Init )
{
	ImGui::SetNextTreeNodeOpen( true, ImGuiCond_Once );
	if ( ImGui::TreeNode( Name ) ) {
		// スライダーの表示.
		...
		// 初期化ボタン.
		if ( CImGuiManager::Button( "Reset" ) ) *Vec3 = Init;
		ImGui::SameLine();
		HelpMarker( u8"初期状態に戻します" );
		ImGui::TreePop();
	}
...
//----------------------------.
void CImGuiManager::InputVec3( const char* Name, D3DXVECTOR3* Vec3, const D3DXVECTOR3& Init )
{
	ImGui::SetNextTreeNodeOpen( true, ImGuiCond_Once );
	if ( ImGui::TreeNode( Name ) ) {
		// ボックスの表示.
...
//----------------------------.
void CImGuiManager::SliderInputVec3( const char* Name, D3DXVECTOR3* Vec3, const float Min, const float Max, const D3DXVECTOR3& Init )
{
	ImGui::SetNextTreeNodeOpen( true, ImGuiCond_Once );
	if ( ImGui::TreeNode( Name ) ) {
		// スライダーの表示.
...
//----------------------------.
void CImGuiManager::SliderVec4( const char* Name, D3DXVECTOR4* Vec4, const float Min, const float Max, const D3DXVECTOR4& Init )
{
	ImGui::SetNextTreeNodeOpen( true, ImGuiCond_Once );
	if ( ImGui::TreeNode( Name ) ) {
		// スライダーの表示.
		...
		// 初期化ボタン.
		if ( CImGuiManager::Button( "Reset" ) ) *Vec4 = Init;
		ImGui::SameLine();
		HelpMarker( u8"初期状態に戻します" );
		ImGui::TreePop();
	}
...
//----------------------------.
void CImGuiManager::InputVec4( const char* Name, D3DXVECTOR4* Vec4, const D3DXVECTOR4& Init )
{
	ImGui::SetNextTreeNodeOpen( true, ImGuiCond_Once );
	if ( ImGui::TreeNode( Name ) ) {
		// ボックスの表示.
...
//----------------------------.
void CImGuiManager::SliderInputVec4( const char* Name, D3DXVECTOR4* Vec4, const float Min, const float Max, const D3DXVECTOR4& Init )
{
	ImGui::SetNextTreeNodeOpen( true, ImGuiCond_Once );
	if ( ImGui::TreeNode( Name ) ) {
		// スライダーの表示.
...
// グループ.
//----------------------------.
void CImGuiManager::Group( const std::function<void()>& Proc, const float Size_y, const float Size_x )
{
	CImGuiManager::Group( Proc, { Size_x, Size_y } );
}
void CImGuiManager::Group( const std::function<void()>& Proc, const D3DXSCALE2& Size )
{
...
// 条件付きのボタン.
//----------------------------.
bool CImGuiManager::Button( const char* Label, D3DXCOLOR4 Color, const D3DXSCALE2& Size )
{
	return CImGuiManager::Button( Label, true, Color, Size );
}
bool CImGuiManager::Button( const char* Label, const bool Flag, D3DXCOLOR4 Color, const D3DXSCALE2& Size )
{
	...
	if ( Flag ) {
		...
		if ( Result					) CSoundManager::PlaySE( "ImguiSE", 0.0f, false, true );
		return Result;
	}
	else {
		// 押せない状態のボタン.
...
// チェックボックス.
//----------------------------.
bool CImGuiManager::CheckBox( const char* Label, bool* v, D3DXCOLOR4 Color )
{
	if ( Color != Color4::None ){
	...
	if ( Oldv	!= *v			)
		CSoundManager::PlaySE( "ImguiSE", 0.0f, false, true );
	return *v;
}
...
// ラジオボタン.
//----------------------------.
std::string CImGuiManager::RadioButton( const char* Label, const std::string& NowItem, const std::vector<std::string>& List )
{
	std::string Now = NowItem;
	return RadioButton( Label, &Now, List );
}
std::string CImGuiManager::RadioButton( const char* Label, std::string* Out, const std::vector<std::string>& List )
{
	// 現在の番号を取得.
	...
	if ( *Out != List[v] ) CSoundManager::PlaySE( "ImguiSE", 0.0f, false, true );
	return *Out = List[v];
}
...
// コンボボックス.
//----------------------------.
std::string CImGuiManager::Combo( const char* Label, const std::string& NowItem, const std::vector<std::string>& List )
{
	std::string Now = NowItem;
	return Combo( Label, &Now, List );
}
std::string CImGuiManager::Combo( const char* Label, std::string* Out, const std::vector<std::string>& List )
{
	// 現在の番号を取得.
	const int& Size = static_cast<int>( List.size() );
	int v = 0;
	...
	if ( *Out != Items[v].second ) CSoundManager::PlaySE( "ImguiSE", 0.0f, false, true );
...
//----------------------------.
ImVec2 CImGuiManager::ConvertVector2ToIm( const D3DXVECTOR2& pSrcVec2Dx )
{
...
//----------------------------.
D3DXVECTOR2 CImGuiManager::ConvertVector2ToDx( const ImVec2& pSrcVec2Im )
{
...
//----------------------------.
ImVec4 CImGuiManager::ConvertVector4ToIm( const D3DXVECTOR4& pSrcVec4Dx )
{
...
//----------------------------.
D3DXVECTOR4 CImGuiManager::ConvertVector4ToDx( const ImVec4& pSrcVec4Im )
{
