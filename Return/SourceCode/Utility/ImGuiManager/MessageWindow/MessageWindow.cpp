...
// インスタンスの取得.
//----------------------------.
CMessageWindow* CMessageWindow::GetInstance()
{
	static std::unique_ptr<CMessageWindow> pInstance = std::make_unique<CMessageWindow>();
...
// 描画.
//-----------------------------.
void CMessageWindow::Render()
{
	CMessageWindow* pI = GetInstance();
...
//----------------------.
void CMessageWindow::PushMessage( const std::string& Message, const D3DXCOLOR3& Color )
{
	PushMessage( Message, Color4::RGBA( Color ) );
}
void CMessageWindow::PushMessage( const std::string& Message, const D3DXCOLOR4& Color )
{
	CMessageWindow* pI = GetInstance();
	...
	[=]() {
		...
		if ( FindString.find( pI->m_FindString ) != std::string::npos ){
			// 文字の表示.
			ImGui::TextColored( CImGuiManager::ConvertVector4ToIm( Color ), Text.c_str() );
		}
	} );
}
...
// メッセージウィンドウ.
//----------------------.
void CMessageWindow::MessageFind()
{
	CMessageWindow* pI = GetInstance();
	...
	ImGui::SameLine();
	CImGuiManager::CheckBox( u8"##MsgWnd_CheckBox", &pI->m_IsFindTime );
	ImGui::SameLine();
	if ( CImGuiManager::Button( u8"削除" ) ) pI->m_MessageList.clear();
	ImGui::Separator();
}
...
//----------------------.
void CMessageWindow::ColorBlock()
{
	CMessageWindow* pI = GetInstance();
	...
	for ( int i = 0; i < EColor::Max; i++ ){
		...
		CImGuiManager::CheckBox( No.c_str(), &Flags[i], Color[i] );
...
// カラーテーブルの設定.
//----------------------.
void CMessageWindow::SetColorTable()
{
