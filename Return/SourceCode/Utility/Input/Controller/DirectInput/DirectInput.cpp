...
{
	static std::unique_ptr<CDirectInput> pInstance = std::make_unique<CDirectInput>();
...
// キーが押されたことを取得する.
//----------------------------.
bool CDirectInput::IsKeyPress( DIRECT_INPUT_KEY Key )
{
...
//----------------------------.
bool CDirectInput::IsKeyDown( DIRECT_INPUT_KEY Key )
{
	CDirectInput* pI = GetInstance();
...
// 離した.
//----------------------------.
bool CDirectInput::IsKeyUp( DIRECT_INPUT_KEY Key )
{
	CDirectInput* pI = GetInstance();
...
// 押し続けている.
//----------------------------.
bool CDirectInput::IsKeyRepeat( DIRECT_INPUT_KEY key )
{
	CDirectInput* pI = GetInstance();
...
// コントローラ接続確認.
//----------------------------.
HRESULT CDirectInput::SettingController()
{
	CDirectInput*	pI = GetInstance();
	HRESULT			hRslt;  // 関数復帰値.
...
// 入力情報回収関数.
//----------------------------.
HRESULT CDirectInput::UpdateKeyInput()
{
	CDirectInput*	pI = GetInstance();
	HRESULT			hRslt;  // 関数復帰値.
...
// 指定キーの入力追加.
//----------------------------.
void CDirectInput::AddKey( DIRECT_INPUT_KEY key )
{
...
//----------------------------.
bool CDirectInput::IsKeyCore( DIRECT_INPUT_KEY key, unsigned int status )
{
