...
// ログの入力.
//----------------------------.
HRESULT CLog::PushLog( const char* Log )
{
	if ( GetInstance()->m_Stop ) return S_OK;
...
HRESULT CLog::PushLog( const std::string& Log )
{
	if ( GetInstance()->m_Stop ) return S_OK;
