...
void CInputSetting::Init()
{
	...
	// 描画処理の設定.
	m_RenderFunc = [&] () {
		ImGui::Text( u8"設定した動作は「CInput」クラスで押した時、離した時など取得できます。" );
		...
		// 新しい行動の追加ボタン.
		if ( CImGuiManager::Button( "+", !m_ActionName.empty() ) ) {
		...
		while ( ActItr != m_KeyBindList.end() ) {
			...
			ImGui::SameLine();
			if ( CImGuiManager::Button( std::string( "x##DeleteButton_" + Name ).c_str() ) ) {
			...
			if ( ImGui::TreeNode( std::string( "##ActionTree_" + Name ).c_str() ) ) {
				...
				// キー入力の追加ボタン.
				if ( CImGuiManager::Button( "+##Action_Key" ) ) {
				...
				while ( KeyItr != KeyBind->Key.end() ) {
					std::string NewKey	= CImGuiManager::Combo( std::string( "KeyCombo##NoName" + std::to_string( No ) ).c_str(), InputList::KeyDataToString( *KeyItr ), InputList::GetKeyList() );
					*KeyItr				= InputList::StringToKeyData( NewKey );
					ImGui::SameLine();

					// キー入力の削除ボタン.
					if ( CImGuiManager::Button( std::string( "x##KeyDelete_" + std::to_string( No ) ).c_str() ) ) {
				...
				// コントローラ入力の追加ボタン.
				if ( CImGuiManager::Button( "+##Action_But" ) ) {
				...
				while ( ButItr != KeyBind->But.end() ) {
					std::string NewBut	= CImGuiManager::Combo( std::string( "ButCombo##NoName" + std::to_string( No ) ).c_str(), InputList::ButDataToString( *ButItr ), InputList::GetButList() );
					*ButItr				= InputList::StringToButData( NewBut );
					ImGui::SameLine();

					// コントローラ入力の削除ボタン.
					if ( CImGuiManager::Button( std::string( "x##ButDelete_" + std::to_string( No ) ).c_str() ) ) {
		...
		// キーバインドの保存ボタン.
		ImGui::Separator();
		if ( CImGuiManager::Button( u8"保存##KeyBindSave" ) ) {
			...
			// キーバインドの更新.
			CInput::SetKeyBind( m_KeyBindList );
		}
	};
