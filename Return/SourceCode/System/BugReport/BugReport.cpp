...
void CBugReport::Init()
{
	...
	m_RenderFunc = [&] () {
		...
		if ( ImGui::TreeNode( "BugReport" ) ) {
			...
			// バグレベルの指定.
			ImGui::SameLine();
			CImGuiManager::Combo( u8"< バグレベル >", &m_BugLv );
			ImGui::SameLine();
			...
			// 出力ボタン.
			if ( CImGuiManager::Button( u8"保存##BugSave", !m_BugTitle.empty() && !m_BugContents.empty() && !m_Discoverer.empty() ) ) {
		...
		if ( ImGui::TreeNode( "BugList" ) ) {
			...
			while ( ListItr != m_BugList.end() ) {
				...
				// 修正完了ボタン.
				if( CImGuiManager::Button( std::string( u8"修正完了##" + ListItr->first ).c_str() ) ) {
				...
				// 削除ボタン.
				if ( CImGuiManager::Button( std::string( u8"削除##" + ListItr->first ).c_str() ) ) {
