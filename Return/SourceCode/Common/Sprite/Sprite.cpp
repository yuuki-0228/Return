...
HRESULT CSprite::Init( const std::string& FilePath )
{
	...
	CLog::PushLog( m_SpriteState.FilePath + " 読み込み : 成功" );
	return S_OK;
}
...
void CSprite::RenderUI( SSpriteRenderState* pRenderState )
{
	...
	// アルファブレンド有効にする.
	if ( !m_DitherFlag ) CDirectX11::SetAlphaBlend( true );

	// プリミティブをレンダリング.
	m_pContext->Draw( 4, 0 );// 板ポリ(頂点4つ分).

	// アルファブレンド無効にする.
	if ( !m_DitherFlag ) CDirectX11::SetAlphaBlend( false );
}
...
void CSprite::Render3D( SSpriteRenderState* pRenderState, const bool IsBillBoard )
{
	...
	// ビルボード用.
	if ( IsBillBoard == true ) {
		D3DXMATRIX CancelRotation = CCameraManager::GetViewMatrix();	// ビュー行列.
		CancelRotation._41
			...
			0, D3D11_MAP_WRITE_DISCARD, 0, &pData ) ) )
	{
		...
		D3DXMATRIX m = mWorld * CCameraManager::GetViewProjMatrix();
		D3DXMatrixTranspose( &m, &m );// 行列を転置する.
		cb.mWVP = m;
...
void CSprite::UIEdtorImGuiRender( const bool IsAutoSave, SSpriteRenderState* pRenderState )
{
	...
	CImGuiManager::InputVec3( "Position",	&pTrance->Position );
	CImGuiManager::InputVec3( "Rotation",	&pDegRot );
	CImGuiManager::InputVec3( "Scale",		&pTrance->Scale, { 1.0f, 1.0f, 1.0f } );
	...
	if ( ImGui::TreeNode( "Stride" ) ) {
		...
		ImGui::SameLine();
		CImGuiManager::HelpMarker( u8"保存後、開きなおすことで反映されます。\n※変更すると、表示サイズは一コマのサイズに変更されます。" );

		// 初期化ボタン.
		if ( CImGuiManager::Button( "Reset" ) ) NewStride = { m_SpriteState.Base.w, m_SpriteState.Base.h };
		ImGui::SameLine();
		CImGuiManager::HelpMarker( u8"初期状態に戻します" );
	...
	if ( ImGui::TreeNode( "LocalPosition" ) ) {
		...
		CImGuiManager::Combo( "##NoName", &SpriteLocalPos, { ELocalPosition::Max } );
		ImGui::SameLine();
		CImGuiManager::HelpMarker( u8"保存後、開きなおすことで反映されます。" );
	...
	if ( ImGui::TreeNode( "UIAnimation" ) ) {
		...
		if ( ImGui::TreeNode( u8"いつ行う？" ) ) {
			CImGuiManager::Combo( "##NoName", &NowWhen, WhenList );
			ImGui::TreePop();
		}
		...
		*UIAnimWhen = ConvertUIAnimationString( NowWhen );
		...
		if ( ImGui::TreeNode( u8"何を行う？" ) ) {
			CImGuiManager::Combo( "##NoName", &NowWhat, WhatList );
			ImGui::TreePop();
		}
	...
	if ( CImGuiManager::Button( "Save" ) ) {
		SpriteStateDataSave( RenderState );
		CMessageWindow::PushMessage( u8"保存しました。", Color4::Magenta );
	}
	ImGui::Separator();
...
void CSprite::HitBoxRender( const D3DXVECTOR4& Color, SSpriteRenderState* pRenderState )
{
	...
	CSpriteResource::GetSprite( "HitBox" )->RenderUI( &HitBoxState );
}
...
bool CSprite::DragUpdate( const bool IsDrag, const bool IsAutoSave, SSpriteRenderState* pRenderState )
{
	...
	const D3DXPOSITION2& MouseOldPos	= CInput::GetMouseOldPosition();
	...
	if ( RenderState->IsGrab ) {
		...
		// 左クリックが離されているか.
		if ( !CKeyInput::IsKeyPress( VK_LBUTTON ) ) {
			// 離した.
			CInput::SetIsMousrGrab( false );
	...
	// つかまれていない.
		...
		// マウスが別のをつかんでいる.
		if ( CInput::IsMousrGrab() ) return false;
		static bool f = false;
		// マウスカーソルに重なっているか.
		if ( GetIsOverlappingTheMouse( pRenderState ) == false ) return false;

		// 左クリックが押された瞬間か.
		if ( !CKeyInput::IsKeyDown( VK_LBUTTON ) ) return false;

		// つかんだ.
		CInput::SetIsMousrGrab( true );
...
HRESULT CSprite::SpriteStateDataLoad()
{
	...
	if ( m_SpriteStateData.empty() ) {
		...
		// 作成できたためもう一度読み込み直す.
		CLog::PushLog( m_SpriteState.FilePath + " をもう一度読み込み直します。" );
		SpriteStateDataLoad();
		return S_OK;
	}
...
HRESULT CSprite::SpriteStateSaveReload()
{
	...
	if ( *m_pIsCreaterLog ) {
		...
		if ( j != m_SpriteStateData ) {
			...
			CLog::PushLog( TextPath + " ファイルのログの作成 : 成功" );
		}
	}

	// スプライト情報の保存.
	if ( FAILED( FileManager::JsonSave( TextPath, m_SpriteStateData ) ) ) return E_FAIL;
	CLog::PushLog( TextPath + " ファイルの作成 : 成功" );
...
bool CSprite::UIAnimPlayCheck( SSpriteRenderState* pRenderState )
{
	...
	// クリックされた時.
		...
		bool IsClick = false;
		if ( AnimData["Click"]["Left"]		) IsClick = CKeyInput::IsKeyDown( VK_LBUTTON );
		if ( AnimData["Click"]["Center"]	) IsClick = CKeyInput::IsKeyDown( VK_MBUTTON );
		if ( AnimData["Click"]["Right"]		) IsClick = CKeyInput::IsKeyDown( VK_RBUTTON );
		return GetIsOverlappingTheMouse( pRenderState ) && IsClick;
	}
	// 特定のキーが押されている時.
	else if ( UIAnimWhen == "Key" ) {
		if ( AnimData["Key"]["Press"] ) {
			if ( AnimData["Key"]["KeyName"]		!= "None" ) return CKeyInput::IsKeyPress(	InputList::StringToKeyData( AnimData["Key"]["KeyName"] ) );
			if ( AnimData["Key"]["ButName"]		!= "None" ) return CXInput::IsKeyPress(		InputList::StringToButData( AnimData["Key"]["ButName"] ) );
			if ( AnimData["Key"]["InputName"]	!= "None" ) return CInput::IsKeyPress(		AnimData["Key"]["InputName"] );
		}
		if ( AnimData["Key"]["Down"] ) {
			if ( AnimData["Key"]["KeyName"]		!= "None" ) return CKeyInput::IsKeyDown(	InputList::StringToKeyData( AnimData["Key"]["KeyName"] ) );
			if ( AnimData["Key"]["ButName"]		!= "None" ) return CXInput::IsKeyDown(		InputList::StringToButData( AnimData["Key"]["ButName"] ) );
			if ( AnimData["Key"]["InputName"]	!= "None" ) return CInput::IsKeyDown(		AnimData["Key"]["InputName"] );
		}
		if ( AnimData["Key"]["Up"] ) {
			if ( AnimData["Key"]["KeyName"]		!= "None" ) return CKeyInput::IsKeyUp(		InputList::StringToKeyData( AnimData["Key"]["KeyName"] ) );
			if ( AnimData["Key"]["ButName"]		!= "None" ) return CXInput::IsKeyUp(		InputList::StringToButData( AnimData["Key"]["ButName"] ) );
			if ( AnimData["Key"]["InputName"]	!= "None" ) return CInput::IsKeyUp(			AnimData["Key"]["InputName"] );
		}
	}
	// メッセージを受け取った時.
	else if ( UIAnimWhen == "GetMessage" ) {
		return CMessage::Check( AnimData["GetMessage"]["Name"] );
	}
	// ランダム( 確率 ).
	else if ( UIAnimWhen == "Random" ) {
		return CRandom::GetRand( 1, 100 ) <= AnimData["Random"]["Probability"];
	}
	return false;
}
...
void CSprite::ScaleUIAnimUpdate( SSpriteRenderState* pRenderState )
{
	...
	if ( UIAnimState->IsLoop == false && IsCnt ) {
		...
		if ( UIAnimState->PlayCnt >= UIAnimState->PlayMax ) {
			...
			if ( !UIAnimState->EndMessage.empty() ) CMessage::Send( UIAnimState->EndMessage, Msg::EMsgType::CheckMsg );
		}
	}
}
...
void CSprite::MoveUIAnimUpdate( SSpriteRenderState* pRenderState )
{
	...
	if ( UIAnimState->Flag_x  )	Pos->x = CEasing::Easing( XEasingName, UIAnimState->Cnt_x, 100.0f, Back.x, StartPos.x,	EndPos.x );
	else						Pos->x = CEasing::Easing( XEasingName, UIAnimState->Cnt_x, 100.0f, Back.x, EndPos.x,	StartPos.x );
	if ( UIAnimState->Flag_y  )	Pos->y = CEasing::Easing( YEasingName, UIAnimState->Cnt_y, 100.0f, Back.y, StartPos.y,	EndPos.y );
	else						Pos->y = CEasing::Easing( YEasingName, UIAnimState->Cnt_y, 100.0f, Back.y, EndPos.y,	StartPos.y );
...
void CSprite::MessageUIAnimUpdate( SSpriteRenderState* pRenderState )
{
	...
	CMessage::Send( AnimData["Message"], Msg::EMsgType::CheckMsg );
}
...
void CSprite::KeyAdvancedSetting( SSpriteRenderState* pRenderState )
{
	...
	if ( ImGui::TreeNode( u8"どのタイミングで行いますか？" ) ) {
		CImGuiManager::RadioButton( "KeyTypeRadio##NoName", &NowButton, ButtonList );
		ImGui::TreePop();
	}
	// 前の状態と今の状態を見て変更されているか.
	...
	if ( ImGui::TreeNode( u8"押されたことを調べるキーの名前" ) ) {
		...
		AnimData["Key"]["KeyName"] = CImGuiManager::Combo("KeyCombo##NoName", KeyName, InputList::GetKeyList());
		ImGui::TreePop();
	}
	ImGui::SetNextTreeNodeOpen( true, ImGuiCond_Once );
	if ( ImGui::TreeNode( u8"押されたことを調べるボタンの名前" ) ) {
		std::string ButName = AnimData["Key"]["ButName"];
		AnimData["Key"]["ButName"] = CImGuiManager::Combo( "ButCombo##NoName", ButName, InputList::GetButList() );
		ImGui::TreePop();
	}
	ImGui::SetNextTreeNodeOpen( true, ImGuiCond_Once );
...
void CSprite::ClickAdvancedSetting( SSpriteRenderState* pRenderState )
{
	...
	if ( ImGui::TreeNode( u8"どのクリックで行いますか？" ) ) {
		CImGuiManager::RadioButton( "ClickTypeRadio##NoName", &NowButton, ClickList );
		ImGui::TreePop();
	}
	// 前の状態と今の状態を見て変更されているか.
	if ( OldButton != NowButton ) {
...
void CSprite::GetMessageAdvancedSetting( SSpriteRenderState* pRenderState )
{
	...
	if ( ImGui::TreeNode( u8"受け取るメッセージ名" ) ) {
		...
		// テスト用のメッセージを送るボタン.
		if( CImGuiManager::Button( u8"メッセージを送る", !MsgName.empty() ) ) {
			CMessage::Send( MsgName, Msg::EMsgType::CheckMsg );
		}
		ImGui::TreePop();
	}
	m_SpriteStateData[RenderState->SceneName][RenderState->No]["UIAnimation"] = AnimData;
...
void CSprite::CreateWhatTempData( SSpriteRenderState* pRenderState )
{
	...
	if ( UIAnimWhat == "Scale" && AnimData["Scale"].empty() ) {
		...
		AnimData["Move"]["End"]["x"]				= RenderState->Transform.Position.x + 50.0f;
		AnimData["Move"]["End"]["y"]				= RenderState->Transform.Position.y;
...
void CSprite::LoopAdvancedSetting( SSpriteRenderState* pRenderState )
{
	...
	if ( ImGui::TreeNode( u8"ループ再生するか" ) ) {
		CImGuiManager::CheckBox( "##IsLoopCheckBox", &UIAnimState->IsLoop );
		ImGui::TreePop();
	}
	...
	ImGui::Separator();
	CImGuiManager::InputInt( u8"再生回数", &UIAnimState->PlayMax );
...
void CSprite::ScaleAdvancedSetting( SSpriteRenderState* pRenderState )
{
	...
	CImGuiManager::InputVec2( u8"最大サイズの設定", &MaxSize, { 1.0f, 1.0f } );
	...
	CImGuiManager::InputVec2( u8"最小サイズの設定", &MinSize );
	...
	CImGuiManager::InputVec2( u8"拡大速度の設定", &ExpansionSpeed, { 0.01f, 0.01f } );
	...
	CImGuiManager::InputVec2( u8"縮小速度の設定", &ShrinkSpeed, { 0.01f, 0.01f } );
	...
	if ( ImGui::TreeNode( u8"x方向の開始時の状態の設定" ) ) {
		CImGuiManager::RadioButton( "##NoName", &NowStart, StartList );
		ImGui::TreePop();
	}
...
void CSprite::AlphaAdvancedSetting( SSpriteRenderState* pRenderState )
{
	...
	// 開始時の透明度の設定.
	CImGuiManager::SliderInputFloat( u8"開始時の透明度", &RenderState->UIAnimState.StartColor.w, 0.0f, 1.0f );

	// 不透明になる速度の設定.
	float OpacitySpeed = AnimData["Alpha"]["Speed"]["Opacity"];
	CImGuiManager::InputFloat( u8"不透明になる速度の設定", &OpacitySpeed, 0.01f );
	AnimData["Alpha"]["Speed"]["Opacity"] = OpacitySpeed;

	// 透明になる速度の設定.
	float ClearSpeed = AnimData["Alpha"]["Speed"]["Clear"];
	CImGuiManager::InputFloat( u8"透明になる速度の設定", &ClearSpeed, 0.01f );
	AnimData["Alpha"]["Speed"]["Clear"] = ClearSpeed;

	// 最大の透明度の設定.
	float MaxAlpha = AnimData["Alpha"]["MaxAlpha"];
	CImGuiManager::SliderInputFloat( u8"最大の透明度の設定", &MaxAlpha, 0.0f, 1.0f, 1.0f );
	AnimData["Alpha"]["MaxAlpha"] = MaxAlpha;

	// 最小の透明度の設定.
	float MinAlpha = AnimData["Alpha"]["MinAlpha"];
	CImGuiManager::SliderInputFloat( u8"最小の透明度の設定", &MinAlpha, 0.0f, 1.0f, 0.0f );
...
void CSprite::RotateAdvancedSetting( SSpriteRenderState* pRenderState )
{
	...
	if ( ImGui::TreeNode( u8"回転方向の設定" ) ) {
		CImGuiManager::RadioButton( "##NoName", &NowDire, DireList );
		ImGui::TreePop();
	}
	...
	CImGuiManager::InputFloat( u8"回転速度の設定", &Speed, 0.1f );
...
void CSprite::MoveAdvancedSetting( SSpriteRenderState* pRenderState )
{
	...
	CImGuiManager::InputVec2( u8"移動終了座標", &MoveEndPos );
	...
	CImGuiManager::InputVec2( u8"移動速度", &Speed );
	...
	CImGuiManager::InputVec2( u8"戻る距離", &Back );
	...
	if ( ImGui::TreeNode( u8"使用するイージング" ) ) {
		...
		CImGuiManager::Combo( u8"X方向", &XEasing, CEasing::GetEasingList() );
		CImGuiManager::Combo( u8"Y方向", &YEasing, CEasing::GetEasingList() );
...
void CSprite::PendulumAdvancedSetting( SSpriteRenderState* pRenderState )
{
	...
	CImGuiManager::InputFloat( u8"速度の設定", &Speed, 0.1f );
	AnimData["Pendulum"]["Speed"] = Speed;

	// 最終角度の設定.
	float EndAng = AnimData["Pendulum"]["EndAng"];
	CImGuiManager::InputFloat( u8"最終角度の設定", &EndAng, 30.0f );
...
void CSprite::AnimationAdvancedSetting( SSpriteRenderState* pRenderState )
{
	...
	CImGuiManager::InputFloat( u8"速度の設定", &Speed, 0.01f );
...
void CSprite::ScrollAdvancedSetting( SSpriteRenderState* pRenderState )
{
	...
	if ( ImGui::TreeNode( u8"UVスクロールを行うか" ) ) {
		...
		CImGuiManager::CheckBox( u8"x方向のUVスクロールを行うか", &RenderState->AnimState.IsUVScrollX );
		AnimData["Scroll"]["IsPlay"]["x"]	= RenderState->AnimState.IsUVScrollX;

		// y方向のUVスクロールを行うか.
		RenderState->AnimState.IsUVScrollY	= AnimData["Scroll"]["IsPlay"]["y"];
		CImGuiManager::CheckBox( u8"y方向のUVスクロールを行うか", &RenderState->AnimState.IsUVScrollY );
	...
	CImGuiManager::InputVec2( u8"スクロール速度の設定", &ScrollSpeed, { 0.01f, 0.01f } );
