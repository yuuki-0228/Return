...
void CFont::RenderUI( const std::string& Text, SFontRenderState* pRenderState )
{
	...
	for ( int i = 0; i < static_cast<int>( Text.length() ); ++i ){
		...
		if ( RenderState->IsOutLine || RenderState->IsBold ) {
			// アルファ値を文字のアルファ値を同じにする.
			RenderState->OutLineColor.w = RenderState->Color.w;
...
{
	// テクスチャの取得.
	ID3D11ShaderResourceView* pTexture = CFontResource::GetFontTexture( m_FileName, c );
			...
			0, D3D11_MAP_WRITE_DISCARD, 0, &pData ) ) )
	{
		...
		// 描画するエリア.
		cb.vRenderArea = pRenderState->RenderArea;
