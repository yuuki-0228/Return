...
// インスタンスの取得.
//----------------------------.
CWindowText* CWindowText::GetInstance()
{
	static std::unique_ptr<CWindowText> pInstance = std::make_unique<CWindowText>();
...
// 初期化.
//----------------------------.
HRESULT CWindowText::Init()
{
	CWindowText* pI = GetInstance();
...
//----------------------------.
HRESULT CWindowText::CreateShader()
{
	CWindowText*	pI				= GetInstance();
	...
	if (FAILED(
		pI->m_pDevice->CreateVertexShader(
	...
	// 頂点インプットレイアウトを作成.
	if (FAILED(
		pI->m_pDevice->CreateInputLayout(
			layout,
			numElements,
			pCompiledShader->GetBufferPointer(),
			pCompiledShader->GetBufferSize(),
	...
	if (FAILED(
		pI->m_pDevice->CreatePixelShader(
			pCompiledShader->GetBufferPointer(),
			pCompiledShader->GetBufferSize(),
			nullptr,
...
// モデル作成.
//----------------------------.
HRESULT CWindowText::CreateModel()
{
	CWindowText* pI = GetInstance();
...
// テクスチャ作成.
//----------------------------.
HRESULT CWindowText::CreateTexture( LPCTSTR lpFileName )
{
	CWindowText* pI = GetInstance();

	// テクスチャ作成.
	if( FAILED( D3DX11CreateShaderResourceViewFromFile(
		pI->m_pDevice,		// リソースを使用するデバイスのポインタ.
		lpFileName,			// ファイル名.
		nullptr, nullptr,
...
// サンプラ作成.
//----------------------------.
HRESULT CWindowText::CreateSampler()
{
	CWindowText* pI = GetInstance();
...
// 描画.
//----------------------------.
void CWindowText::Render( const std::string& text, const D3DXPOSITION3& Pos, const float FontSize, const D3DXCOLOR4& Color )
{
	CWindowText::Render( text, static_cast<int>( Pos.x ), static_cast<int>( Pos.y ), FontSize, Color );
}
void CWindowText::Render( const std::string& text, const D3DXPOSITION2& Pos, const float FontSize, const D3DXCOLOR4& Color )
{
	CWindowText::Render( text, static_cast<int>( Pos.x ), static_cast<int>( Pos.y ), FontSize, Color );
}
void CWindowText::Render( const std::string& text, const int x, const int y, const float FontSize, const D3DXCOLOR4& Color )
{
	CWindowText* pI = GetInstance();
...
void CWindowText::DebugRender( const std::string& text, const D3DXPOSITION3& Pos, const float FontSize, const D3DXCOLOR4& Color )
{
#ifdef _DEBUG
	Render( text, static_cast<int>( Pos.x ), static_cast<int>( Pos.y ), FontSize, Color );
#endif	// #ifdef _DEBUG.
}
void CWindowText::DebugRender( const std::string& text, const D3DXPOSITION2& Pos, const float FontSize, const D3DXCOLOR4& Color )
{
#ifdef _DEBUG
	Render( text, static_cast<int>( Pos.x ), static_cast<int>( Pos.y ), FontSize, Color );
#endif	// #ifdef _DEBUG.
}
void CWindowText::DebugRender( const std::string& text, const int x, const int y, const float FontSize, const D3DXCOLOR4& Color )
{
#ifdef _DEBUG
	...
	int FontIndex, int x, int y )
{
			...
			0, D3D11_MAP_WRITE_DISCARD, 0, &pData)))
	{
		...
		// ビューポートの幅、高さを渡す.
		cb.fViewPortWidth	= static_cast<float>( CDirectX11::GetWndWidth()		);
		cb.fViewPortHeight	= static_cast<float>( CDirectX11::GetWndHeight()	);
