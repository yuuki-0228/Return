...
// テクスチャの変更.
//----------------------------.
void CStaticMesh::ChangeTexture( const int No )
{
	// 現在のテクスチャの破棄.
	DetachTexture();

	// テクスチャの設定.
	AttachTexture( CreateTexture( m_TexturePathList[No] ) );
}
...
void CStaticMesh::Render( STransform* pTransform, const D3DXMATRIX* pRot )
{
		...
		0, &pData)))
	{
		...
		// ライト方向.
		const D3DXVECTOR3 Direction = CLight::GetDirection();
	...
	D3DXMATRIX& mWorld )
{
	...
	for (DWORD No = 0; No < m_NumAttr; No++)
	{
		...
		// フォグの描画.
		if ( m_IsFog == true ) m_pFog->Render( m_pMaterials->dwNumFace * 3, mWorld, m_pMaterials->pTexture, 0.0f, 100.0f );
...
// テクスチャの作成.
//---------------------------.
ID3D11ShaderResourceView* CStaticMesh::CreateTexture( const std::string& FilePath )
{
...
// 差し替え用のテクスチャを作成.
//---------------------------.
HRESULT CStaticMesh::CreateChangeTexture( const std::string& FilePath )
{
	// テクスチャリストのファイルパスを作成.
