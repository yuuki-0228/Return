...
namespace {
	// シェーダファイル名(ディレクトリも含む).
	const TCHAR SHADER_NAME[] = _T( "Data\\Shader\\Collision.hlsl" );
}
...
// 初期化関数.
//----------------------------.
HRESULT CCollisionMesh::Init( LPCTSTR lpFileName )
{
	// ファイル読み込み.
...
// メッシュ読み込み.
//----------------------------.
HRESULT CCollisionMesh::LoadXMesh( LPCTSTR lpFileName )
{
	// ファイル名をコピー.
...
// モデル作成.
//----------------------------.
HRESULT CCollisionMesh::CreateModel()
{
	// マテリアル作成.
...
// マテリアル作成.
//----------------------------.
HRESULT CCollisionMesh::CreateMaterials()
{
	// ファイルのパス(ディレクトリ)を確保.
...
// インデックスバッファ作成.
//----------------------------.
HRESULT CCollisionMesh::CreateIndexBuffer()
{
...
// 頂点バッファ作成.
//----------------------------.
HRESULT CCollisionMesh::CreateVertexBuffer()
{
	D3D11_BUFFER_DESC		bd;			// Dx11バッファ構造体.
	D3D11_SUBRESOURCE_DATA	InitData;	// 初期化データ.
...
// サンプラ作成.
HRESULT CCollisionMesh::CreateSampler()
{
	// テクスチャ用のサンプラ構造体.
...
//----------------------------.
HRESULT CCollisionMesh::CreateShader()
{
...
// コンスタントバッファ作成.
//----------------------------.
HRESULT CCollisionMesh::CreateConstantBuffer()
{
	// コンスタントバッファ(メッシュ用).
...
//----------------------------.
void CCollisionMesh::Render( STransform* pTransform, const D3DXMATRIX* pRot )
{
...
//----------------------------.
void CCollisionMesh::RenderMesh(
	D3DXMATRIX& mWorld )
{
	// シェーダのコンスタントバッファに各種データを渡す.
	D3D11_MAPPED_SUBRESOURCE pData;
