...
namespace{
	// 描画スプライト最大数.
	const int RENDER_SPRITE_MAX		= 3000;
	// エフェクト管理用インスタンス最大数.
...
HRESULT CEffect::Create()
{
	...
	m_pRenderer = ::EffekseerRendererDX11::Renderer::Create(
		CDirectX11::GetDevice(), CDirectX11::GetContext(), RENDER_SPRITE_MAX );
...
HRESULT CEffect::LoadData( const std::string& FileName )
{
	...
	// エフェクトの読込.
	const std::wstring wName = StringConversion::to_wString( FileName );
