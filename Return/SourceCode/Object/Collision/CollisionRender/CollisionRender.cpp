...
{
#ifdef _DEBUG
	CCollisionRender* pI = GetInstance();

	// 当たり判定を表示するか.
	if ( CDebugKeyInput::IsKeyPress( VK_CONTROL ) ) {
		if ( CDebugKeyInput::IsKeyDown( VK_F1 ) ) pI->m_IsPointRender.Inversion();		// 点の表示/非表示.
		if ( CDebugKeyInput::IsKeyDown( VK_F2 ) ) pI->m_IsSphereRender.Inversion();		// 球体の表示/非表示.
		if ( CDebugKeyInput::IsKeyDown( VK_F3 ) ) pI->m_IsCylinderRender.Inversion();	// 円柱の表示/非表示.
		if ( CDebugKeyInput::IsKeyDown( VK_F4 ) ) pI->m_IsRayRender.Inversion();		// レイの表示/非表示.
		if ( CDebugKeyInput::IsKeyDown( VK_F5 ) ) pI->m_IsMeshRender.Inversion();		// メッシュの表示/非表示.
		if ( CDebugKeyInput::IsKeyDown( VK_F6 ) ) pI->m_IsPoint2DRender.Inversion();	// 点2Dの表示/非表示.
		if ( CDebugKeyInput::IsKeyDown( VK_F7 ) ) pI->m_IsBox2DRender.Inversion();		// ボックス2Dの表示/非表示.
		if ( CDebugKeyInput::IsKeyDown( VK_F8 ) ) pI->m_IsSphere2DRender.Inversion();	// 円2Dの表示/非表示.
	}
...
// 点の追加.
//---------------------------.
void CCollisionRender::PushPoint( const D3DXPOSITION3& pPos )
{
#ifdef _DEBUG
	CCollisionRender* pI = GetInstance();
...
//---------------------------.
void CCollisionRender::PushCylinder( CCylinder* pCylinder )
{
#ifdef _DEBUG
	CCollisionRender* pI = GetInstance();
...
//---------------------------.
void CCollisionRender::PushRay( CRay* pRay )
{
#ifdef _DEBUG
	CCollisionRender* pI = GetInstance();
...
//	Release / レイを描画しない場合は処理は行わない.
//---------------------------.
void CCollisionRender::PushRay( CCrossRay* pRay )
{
#ifdef _DEBUG
	CCollisionRender* pI = GetInstance();
...
//---------------------------.
void CCollisionRender::PushMesh( CStaticMesh* pMesh, STransform* pTrans )
{
#ifdef _DEBUG
	CCollisionRender* pI = GetInstance();
...
void CCollisionRender::PushMesh( CCollisionMesh* pMesh, STransform* pTrans )
{
#ifdef _DEBUG
	CCollisionRender* pI = GetInstance();
...
//	点を描画しない場合は処理は行わない.
//---------------------------.
void CCollisionRender::PushPoint2D( const D3DXPOSITION3& pPos )
{
	PushPoint2D( { pPos.x, pPos.y } );
}
void CCollisionRender::PushPoint2D( const D3DXPOSITION2& pPos )
{
#ifdef _DEBUG
	CCollisionRender* pI = GetInstance();
...
//---------------------------.
void CCollisionRender::PushBox2D( CBox2D* pBox2D )
{
#ifdef _DEBUG
	CCollisionRender* pI = GetInstance();
...
//	円2Dを描画しない場合は処理は行わない.
//---------------------------.
void CCollisionRender::PushSphere2D( CSphere2D* pSphere2D )
{
#ifdef _DEBUG
	CCollisionRender* pI = GetInstance();
...
//	点を描画しない場合は処理は行わない.
//---------------------------.
void CCollisionRender::PointRender()
{
	CCollisionRender* pI = GetInstance();
	...
	if ( pI->m_pSphereMesh == nullptr ) {
		pI->m_pSphereMesh = CMeshResource::GetCollision( "Sphere_c" );
	}
...
void CCollisionRender::SphereRender()
{
	...
	// 当たり判定を描画していく.
	CDirectX11::SetRasterizerState( ERS_STATE::Wire );
...
//	円柱を描画しない場合処理は行わない.
//---------------------------.
void CCollisionRender::CylinderRender()
{
	CCollisionRender* pI = GetInstance();
	...
	if ( pI->m_pCylinderMesh == nullptr ) {
		pI->m_pCylinderMesh = CMeshResource::GetCollision( "Cylinder_c" );
	}
...
//	メッシュを描画しない場合処理は行わない.
//---------------------------.
void CCollisionRender::MeshRender()
{
	CCollisionRender* pI = GetInstance();
...
//	点2Dを描画しない場合処理は行わない.
//---------------------------.
void CCollisionRender::Point2DRender()
{
	CCollisionRender* pI = GetInstance();
	...
	if ( pI->m_pPoint2DSprite == nullptr ) {
		pI->m_pPoint2DSprite = CSpriteResource::GetSprite( "PointColl2D", &pI->m_Point2DState );
	}
...
//---------------------------.
void CCollisionRender::Box2DRender()
{
	CCollisionRender* pI = GetInstance();
	...
	if ( pI->m_pBox2DSprite == nullptr ) {
		pI->m_pBox2DSprite = CSpriteResource::GetSprite( "BoxColl2D", &pI->m_Box2DState );
	}
...
//	円2Dを描画しない場合処理は行わない.
//---------------------------.
void CCollisionRender::Sphere2DRender()
{
	CCollisionRender* pI = GetInstance();
	...
	if ( pI->m_pSphere2DSprite == nullptr ) {
		pI->m_pSphere2DSprite = CSpriteResource::GetSprite( "SphereColl2D", &pI->m_Sphere2DState );
	}
