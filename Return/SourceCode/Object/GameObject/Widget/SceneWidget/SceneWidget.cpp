...
void CSceneWidget::UIEdtorImGuiRender( const bool& IsAutoSave )
{
	...
	for ( auto itr = m_RenderList.rbegin(), e = m_RenderList.rend(); itr != e; ++itr ) {
		...
		// 削除ボタン.
		if ( CImGuiManager::Button( std::string( "x##Del_" + Key + std::to_string( No ) ).c_str(), Color4::Red ) ) {
			// シーン情報を保存する.
			if ( IsAutoSave ) {
				if ( FAILED( AllSpriteStateDataSave() ) ) return;
				CLog::PushLog( "自動で" + m_SceneName + "シーンのスプライト情報を全て保存しました。" );
			}
			// 指定したスプライトの削除.
		...
		// 上に移動ボタン.
		ImGui::SameLine();
		if ( CImGuiManager::Button( std::string( "^##Up_" + Key + std::to_string( No ) ).c_str() ) ) {
		...
		// 下に移動ボタン.
		ImGui::SameLine();
		if ( CImGuiManager::Button( std::string( "v##Down_" + Key + std::to_string( No ) ).c_str() ) ) {
			m_SpriteState[Key][No].UIAnimState.StartTransform.Position.z	-= 0.001f;
			RenderSort();
		}
		...
		CImGuiManager::CheckBox( std::string( "##LockCheckbox_" + Key + std::to_string( No ) ).c_str(), &Flag, Color4::Gray );
		...
		if ( ImGui::TreeNode( std::string( Key + "_" + std::to_string( No ) ).c_str() ) ) {
			ImGui::Separator();
			CImGuiManager::CheckBox( std::string( u8"表示するか##Checkbox_" + Key + std::to_string( No ) ).c_str(), &m_SpriteState[Key][No].IsDisp );
...
HRESULT CSceneWidget::AllSpriteStateDataSave()
{
	...
	for ( auto& l : m_DeleteList ) {
		CSpriteResource::GetSprite( l )->SpriteStateSaveReload();
	}
	m_DeleteList.clear();
...
{
	const std::vector<std::string>& Resource = CSpriteResource::GetSpriteNames();

	// このシーンで読み込む画像を取得する.
	for ( auto& r : Resource ) {
		CSprite*				 pSprite	= CSpriteResource::GetSprite( r );
